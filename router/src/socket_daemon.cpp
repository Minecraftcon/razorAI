...
void SocketDaemon::LogMessage(const std::string& severity, const std::string& message) {
    // Write to daemon.log (legacy, human-readable daemon log)
    const char* home = std::getenv("HOME");
    ...
    // Also mirror to unified debug.log at the appropriate level
    if (severity == "ERROR") {
        RLOG_ERROR("[daemon] " << message);
    } else if (severity == "WARN" || severity == "WARNING") {
        RLOG_WARN("[daemon] " << message);
    } else if (severity == "DEBUG") {
        RLOG_DEBUG("[daemon] " << message);
    } else {
        RLOG_INFO("[daemon] " << message);
    }
}
...
std::string SocketDaemon::ProcessRequestJson(const std::string& request_json, int client_fd) {
...

    {
        ...
        sys_prompts.push_back(
            ...
            "EXECUTION MODEL (Reason → Parallel Act → Observe):\n"
            "1. REASON: Before calling any tool, write 1-2 sentences explaining WHAT you are about to do and WHY.\n"
            "   Example: 'I will check all three directories at once to find where the config is stored.'\n"
            "2. ACT — PARALLEL TOOL CALLS (MANDATORY PREFERENCE):\n"
            "   * When multiple INDEPENDENT tool calls are needed in the same step, YOU MUST issue ALL of them together in a single response.\n"
            "   * This is especially required for: reading multiple files, listing multiple directories, running non-blocking lookups, and any repetitive data-gathering tasks.\n"
            "   * CORRECT (preferred): reason text once → list_dir A + list_dir B + list_dir C (all at once)\n"
            "   * WRONG (discouraged): reason → list_dir A → wait → reason → list_dir B → wait → reason → list_dir C\n"
            "   * Only call tools sequentially if a later call depends on the result of an earlier one.\n"
            "3. OBSERVE: After tool results arrive, analyze all of them together and draw conclusions or proceed.\n\n"
            ...
            "TOOL STRATEGY:\n"
            "- PARALLEL BATCHING (HIGHEST PRIORITY RULE):\n"
            "  * For any set of INDEPENDENT reads, listings, or lookups — ALWAYS batch them into one turn.\n"
            "  * Ideal cases for parallel tool calls: reading multiple source files, listing multiple directories,\n"
            "    checking multiple configs, running multiple non-blocking queries.\n"
            "  * Pattern — STRONGLY PREFERRED:\n"
            "      'Now I will inspect all relevant files at once.'\n"
            "      → read_file(A)  read_file(B)  read_file(C)   [all in the same turn]\n"
            "  * Pattern — DISCOURAGED (only use when B depends on A's result):\n"
            "      → read_file(A) → wait → read_file(B) → wait → read_file(C)\n"
            "  * Violating this rule by doing one-tool-per-turn on independent tasks is WRONG behavior.\n\n"
    ...
    if (route_res.cache_hit && !route_res.cached_response.empty()) {
        ...
        // Prevent sudden stops by specifying max_tokens
        ...
        if (provider == "opencode" || endpoint == "opencode") {
            std::string opencode_bin = "/home/shado/.opencode/bin/opencode";
            if (access(opencode_bin.c_str(), X_OK) != 0) {
                opencode_bin = "opencode";
            }
            std::string effective_key = api_key.empty() ? (getenv("OPENCODE_API_KEY") ? getenv("OPENCODE_API_KEY") : "") : api_key;
            RLOG_INFO("OpenCode CLI launch (secure)");
            ...
            if (pipe(pipefd) == 0) {
                ...
                if (pid == 0) {
                    ...
                    setenv("OPENCODE_API_KEY", effective_key.c_str(), 1);

                    std::vector<char*> args;
                    args.push_back((char*)opencode_bin.c_str());
                    args.push_back((char*)"run");
                    args.push_back((char*)"-m");
                    args.push_back((char*)model_id.c_str());
                    args.push_back((char*)prompt.c_str());
                    args.push_back(nullptr);

                    execvp(opencode_bin.c_str(), args.data());
                    exit(1);
                } else if (pid > 0) {
                    close(pipefd[1]);
                    FILE* out_pipe = fdopen(pipefd[0], "r");
                    if (out_pipe) {
                        char buf[512];
                        while (fgets(buf, sizeof(buf), out_pipe) != nullptr) {
                            std::string line_chunk = buf;
                            if (line_chunk.rfind("> build", 0) == 0) continue;
                            streamed_content += line_chunk;
                            json sc;
                            sc["status"] = "stream_chunk";
                            sc["token"] = line_chunk;
                            std::string sc_line = sc.dump() + "\n";
                            if (!WriteAll(client_fd, sc_line)) break;
                        }
                        fclose(out_pipe);
                    }
                    int status;
                ...
                RLOG_ERROR("OpenCode CLI pipe() failed");
            }
        } else {
        raw_api_body = HttpClient::PostStream(endpoint, api_key, payload_str, [&](const std::string& sse_data) {
            try {
                    ...
                    if (!r_tok.empty()) {
                        ...
                        sc["token"] = r_tok;
                        std::string sc_line = sc.dump() + "\n";
                        if (!WriteAll(client_fd, sc_line)) return;
                    }
        ...
        } // end else (non-opencode HTTP stream)
        ...
        if (got_tool_calls_signal && !tc_names.empty()) {
            ...
            // Reasoning tokens were already streamed live to the UI.
            // Do NOT re-send them as the final response — just leave response_text empty.
            // The UI will see an empty response and call ProvideResponse("") to unfreeze.
            response_text = "";
        } else {
        ...
        if ((response_text.find("[Router Output]") != std::string::npos || response_text.empty()) && !raw_api_body.empty()) {
            RLOG_INFO("SocketDaemon::ProcessRequestJson raw API body | model=" << model_id
                      << " | body_len=" << raw_api_body.size()
                      << " | body=\n" << raw_api_body);
        }
